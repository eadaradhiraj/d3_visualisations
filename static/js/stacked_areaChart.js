var data1 = [{
    date: 1517529600000,
    negative: 165.0,
    neutral: 130.0,
    positive: 385.0,
    total: 680.0
  },
  {
    date: 1517616000000,
    negative: 130.0,
    neutral: 35.0,
    positive: 185.0,
    total: 350.0
  },
  {
    date: 1518307200000,
    negative: 85.0,
    neutral: 80.0,
    positive: 145.0,
    total: 310.0
  },
  {
    date: 1517702400000,
    negative: 50.0,
    neutral: 50.0,
    positive: 180.0,
    total: 280.0
  },
  {
    date: 1517270400000,
    negative: 95.0,
    neutral: 25.0,
    positive: 150.0,
    total: 270.0
  },
  {
    date: 1523491200000,
    negative: 80.0,
    neutral: 20.0,
    positive: 168.0,
    total: 268.0
  },
  {
    date: 1517443200000,
    negative: 70.0,
    neutral: 20.0,
    positive: 175.0,
    total: 265.0
  },
  {
    date: 1517356800000,
    negative: 80.0,
    neutral: 10.0,
    positive: 165.0,
    total: 255.0
  },
  {
    date: 1517184000000,
    negative: 35.0,
    neutral: 45.0,
    positive: 175.0,
    total: 255.0
  },
  {
    date: 1517875200000,
    negative: 95.0,
    neutral: 45.0,
    positive: 110.0,
    total: 250.0
  },
  {
    date: 1517788800000,
    negative: 80.0,
    neutral: 50.0,
    positive: 115.0,
    total: 245.0
  },
  {
    date: 1519257600000,
    negative: 52.0,
    neutral: 28.0,
    positive: 156.0,
    total: 236.0
  },
  {
    date: 1518998400000,
    negative: 70.0,
    neutral: 80.0,
    positive: 80.0,
    total: 230.0
  },
  {
    date: 1517961600000,
    negative: 75.0,
    neutral: 45.0,
    positive: 90.0,
    total: 210.0
  },
  {
    date: 1518220800000,
    negative: 75.0,
    neutral: 15.0,
    positive: 120.0,
    total: 210.0
  },
  {
    date: 1518134400000,
    negative: 80.0,
    neutral: 25.0,
    positive: 105.0,
    total: 210.0
  },
  {
    date: 1517097600000,
    negative: 33.0,
    neutral: 51.0,
    positive: 120.0,
    total: 204.0
  },
  {
    date: 1518480000000,
    negative: 75.0,
    neutral: 25.0,
    positive: 100.0,
    total: 200.0
  },
  {
    date: 1518048000000,
    negative: 75.0,
    neutral: 15.0,
    positive: 105.0,
    total: 195.0
  },
  {
    date: 1519430400000,
    negative: 36.0,
    neutral: 8.0,
    positive: 148.0,
    total: 192.0
  },
  {
    date: 1518825600000,
    negative: 65.0,
    neutral: 0.0,
    positive: 120.0,
    total: 185.0
  },
  {
    date: 1519344000000,
    negative: 80.0,
    neutral: 12.0,
    positive: 92.0,
    total: 184.0
  },
  {
    date: 1519516800000,
    negative: 40.0,
    neutral: 40.0,
    positive: 104.0,
    total: 184.0
  },
  {
    date: 1519084800000,
    negative: 90.0,
    neutral: 5.0,
    positive: 80.0,
    total: 175.0
  },
  {
    date: 1518393600000,
    negative: 110.0,
    neutral: 15.0,
    positive: 50.0,
    total: 175.0
  },
  {
    date: 1518912000000,
    negative: 55.0,
    neutral: 20.0,
    positive: 95.0,
    total: 170.0
  },
  {
    date: 1519171200000,
    negative: 47.0,
    neutral: 14.0,
    positive: 87.0,
    total: 148.0
  },
  {
    date: 1518566400000,
    negative: 55.0,
    neutral: 15.0,
    positive: 70.0,
    total: 140.0
  },
  {
    date: 1518652800000,
    negative: 65.0,
    neutral: 10.0,
    positive: 60.0,
    total: 135.0
  },
  {
    date: 1518739200000,
    negative: 50.0,
    neutral: 5.0,
    positive: 75.0,
    total: 130.0
  },
  {
    date: 1519603200000,
    negative: 36.0,
    neutral: 16.0,
    positive: 76.0,
    total: 128.0
  },
  {
    date: 1519689600000,
    negative: 40.0,
    neutral: 12.0,
    positive: 72.0,
    total: 124.0
  },
  {
    date: 1516838400000,
    negative: 48.0,
    neutral: 22.0,
    positive: 48.0,
    total: 118.0
  },
  {
    date: 1516752000000,
    negative: 36.0,
    neutral: 18.0,
    positive: 62.0,
    total: 116.0
  },
  {
    date: 1522108800000,
    negative: 33.0,
    neutral: 5.0,
    positive: 71.0,
    total: 109.0
  },
  {
    date: 1515110400000,
    negative: 18.0,
    neutral: 14.0,
    positive: 76.0,
    total: 108.0
  },
  {
    date: 1515024000000,
    negative: 16.0,
    neutral: 20.0,
    positive: 70.0,
    total: 106.0
  },
  {
    date: 1522800000000,
    negative: 29.0,
    neutral: 8.0,
    positive: 53.0,
    total: 90.0
  },
  {
    date: 1515542400000,
    negative: 31.0,
    neutral: 8.0,
    positive: 49.0,
    total: 88.0
  },
  {
    date: 1522454400000,
    negative: 19.0,
    neutral: 15.0,
    positive: 50.0,
    total: 84.0
  },
  {
    date: 1522886400000,
    negative: 37.0,
    neutral: 7.0,
    positive: 40.0,
    total: 84.0
  },
  {
    date: 1516147200000,
    negative: 33.0,
    neutral: 13.0,
    positive: 37.0,
    total: 83.0
  },
  {
    date: 1521676800000,
    negative: 20.0,
    neutral: 10.0,
    positive: 52.0,
    total: 82.0
  },
  {
    date: 1521590400000,
    negative: 29.0,
    neutral: 12.0,
    positive: 40.0,
    total: 81.0
  },
  {
    date: 1522972800000,
    negative: 38.0,
    neutral: 4.0,
    positive: 36.0,
    total: 78.0
  },
  {
    date: 1521763200000,
    negative: 18.0,
    neutral: 6.0,
    positive: 53.0,
    total: 77.0
  },
  {
    date: 1515196800000,
    negative: 14.0,
    neutral: 12.0,
    positive: 50.0,
    total: 76.0
  },
  {
    date: 1516060800000,
    negative: 19.0,
    neutral: 13.0,
    positive: 44.0,
    total: 76.0
  },
  {
    date: 1515715200000,
    negative: 13.0,
    neutral: 14.0,
    positive: 48.0,
    total: 75.0
  },
  {
    date: 1522713600000,
    negative: 44.0,
    neutral: 6.0,
    positive: 24.0,
    total: 74.0
  },
  {
    date: 1521158400000,
    negative: 29.0,
    neutral: 16.0,
    positive: 29.0,
    total: 74.0
  },
  {
    date: 1523404800000,
    negative: 26.0,
    neutral: 10.0,
    positive: 38.0,
    total: 74.0
  },
  {
    date: 1516665600000,
    negative: 8.0,
    neutral: 6.0,
    positive: 60.0,
    total: 74.0
  },
  {
    date: 1522281600000,
    negative: 30.0,
    neutral: 13.0,
    positive: 28.0,
    total: 71.0
  },
  {
    date: 1522195200000,
    negative: 30.0,
    neutral: 7.0,
    positive: 33.0,
    total: 70.0
  },
  {
    date: 1516924800000,
    negative: 20.0,
    neutral: 0.0,
    positive: 50.0,
    total: 70.0
  },
  {
    date: 1514937600000,
    negative: 17.0,
    neutral: 18.0,
    positive: 35.0,
    total: 70.0
  },
  {
    date: 1515456000000,
    negative: 23.0,
    neutral: 7.0,
    positive: 40.0,
    total: 70.0
  },
  {
    date: 1515888000000,
    negative: 21.0,
    neutral: 23.0,
    positive: 25.0,
    total: 69.0
  },
  {
    date: 1523318400000,
    negative: 16.0,
    neutral: 14.0,
    positive: 39.0,
    total: 69.0
  },
  {
    date: 1522368000000,
    negative: 34.0,
    neutral: 10.0,
    positive: 24.0,
    total: 68.0
  },
  {
    date: 1520726400000,
    negative: 19.0,
    neutral: 9.0,
    positive: 38.0,
    total: 66.0
  },
  {
    date: 1522627200000,
    negative: 33.0,
    neutral: 12.0,
    positive: 21.0,
    total: 66.0
  },
  {
    date: 1515628800000,
    negative: 16.0,
    neutral: 5.0,
    positive: 45.0,
    total: 66.0
  },
  {
    date: 1521072000000,
    negative: 28.0,
    neutral: 12.0,
    positive: 26.0,
    total: 66.0
  },
  {
    date: 1521244800000,
    negative: 16.0,
    neutral: 7.0,
    positive: 42.0,
    total: 65.0
  },
  {
    date: 1523059200000,
    negative: 26.0,
    neutral: 14.0,
    positive: 21.0,
    total: 61.0
  },
  {
    date: 1515801600000,
    negative: 18.0,
    neutral: 2.0,
    positive: 41.0,
    total: 61.0
  },
  {
    date: 1523232000000,
    negative: 12.0,
    neutral: 18.0,
    positive: 31.0,
    total: 61.0
  },
  {
    date: 1515283200000,
    negative: 17.0,
    neutral: 21.0,
    positive: 22.0,
    total: 60.0
  },
  {
    date: 1514851200000,
    negative: 16.0,
    neutral: 8.0,
    positive: 36.0,
    total: 60.0
  },
  {
    date: 1520812800000,
    negative: 5.0,
    neutral: 8.0,
    positive: 46.0,
    total: 59.0
  },
  {
    date: 1515369600000,
    negative: 23.0,
    neutral: 10.0,
    positive: 25.0,
    total: 58.0
  },
  {
    date: 1520985600000,
    negative: 19.0,
    neutral: 8.0,
    positive: 30.0,
    total: 57.0
  },
  {
    date: 1516233600000,
    negative: 12.0,
    neutral: 8.0,
    positive: 34.0,
    total: 54.0
  },
  {
    date: 1523145600000,
    negative: 20.0,
    neutral: 10.0,
    positive: 23.0,
    total: 53.0
  },
  {
    date: 1521936000000,
    negative: 22.0,
    neutral: 11.0,
    positive: 20.0,
    total: 53.0
  },
  {
    date: 1521504000000,
    negative: 22.0,
    neutral: 5.0,
    positive: 26.0,
    total: 53.0
  },
  {
    date: 1519948800000,
    negative: 21.0,
    neutral: 4.0,
    positive: 27.0,
    total: 52.0
  },
  {
    date: 1521849600000,
    negative: 21.0,
    neutral: 2.0,
    positive: 28.0,
    total: 51.0
  },
  {
    date: 1522022400000,
    negative: 17.0,
    neutral: 5.0,
    positive: 27.0,
    total: 49.0
  },
  {
    date: 1520208000000,
    negative: 15.0,
    neutral: 13.0,
    positive: 21.0,
    total: 49.0
  },
  {
    date: 1522540800000,
    negative: 19.0,
    neutral: 6.0,
    positive: 22.0,
    total: 47.0
  },
  {
    date: 1516492800000,
    negative: 12.0,
    neutral: 10.0,
    positive: 25.0,
    total: 47.0
  },
  {
    date: 1520640000000,
    negative: 9.0,
    neutral: 2.0,
    positive: 35.0,
    total: 46.0
  },
  {
    date: 1521417600000,
    negative: 16.0,
    neutral: 3.0,
    positive: 26.0,
    total: 45.0
  },
  {
    date: 1516320000000,
    negative: 19.0,
    neutral: 7.0,
    positive: 17.0,
    total: 43.0
  },
  {
    date: 1520553600000,
    negative: 12.0,
    neutral: 14.0,
    positive: 17.0,
    total: 43.0
  },
  {
    date: 1521331200000,
    negative: 17.0,
    neutral: 2.0,
    positive: 21.0,
    total: 40.0
  },
  {
    date: 1520467200000,
    negative: 9.0,
    neutral: 11.0,
    positive: 19.0,
    total: 39.0
  },
  {
    date: 1516579200000,
    negative: 12.0,
    neutral: 5.0,
    positive: 22.0,
    total: 39.0
  },
  {
    date: 1523923200000,
    negative: 8.0,
    neutral: 11.0,
    positive: 19.0,
    total: 38.0
  },
  {
    date: 1525046400000,
    negative: 6.0,
    neutral: 25.0,
    positive: 6.0,
    total: 37.0
  },
  {
    date: 1525305600000,
    negative: 19.0,
    neutral: 8.0,
    positive: 10.0,
    total: 37.0
  },
  {
    date: 1519776000000,
    negative: 21.0,
    neutral: 4.0,
    positive: 11.0,
    total: 36.0
  },
  {
    date: 1525219200000,
    negative: 8.0,
    neutral: 12.0,
    positive: 13.0,
    total: 33.0
  },
  {
    date: 1524787200000,
    negative: 7.0,
    neutral: 9.0,
    positive: 17.0,
    total: 33.0
  },
  {
    date: 1516406400000,
    negative: 10.0,
    neutral: 2.0,
    positive: 19.0,
    total: 31.0
  },
  {
    date: 1520380800000,
    negative: 10.0,
    neutral: 3.0,
    positive: 18.0,
    total: 31.0
  },
  {
    date: 1523577600000,
    negative: 5.0,
    neutral: 6.0,
    positive: 18.0,
    total: 29.0
  },
  {
    date: 1514764800000,
    negative: 6.0,
    neutral: 3.0,
    positive: 20.0,
    total: 29.0
  },
  {
    date: 1524528000000,
    negative: 7.0,
    neutral: 9.0,
    positive: 12.0,
    total: 28.0
  },
  {
    date: 1523664000000,
    negative: 7.0,
    neutral: 16.0,
    positive: 4.0,
    total: 27.0
  },
  {
    date: 1519862400000,
    negative: 9.0,
    neutral: 2.0,
    positive: 16.0,
    total: 27.0
  },
  {
    date: 1525392000000,
    negative: 6.0,
    neutral: 7.0,
    positive: 12.0,
    total: 25.0
  },
  {
    date: 1520899200000,
    negative: 4.0,
    neutral: 2.0,
    positive: 19.0,
    total: 25.0
  },
  {
    date: 1520294400000,
    negative: 9.0,
    neutral: 3.0,
    positive: 10.0,
    total: 22.0
  },
  {
    date: 1524614400000,
    negative: 6.0,
    neutral: 7.0,
    positive: 9.0,
    total: 22.0
  },
  {
    date: 1524268800000,
    negative: 5.0,
    neutral: 5.0,
    positive: 11.0,
    total: 21.0
  },
  {
    date: 1524182400000,
    negative: 4.0,
    neutral: 7.0,
    positive: 10.0,
    total: 21.0
  },
  {
    date: 1524700800000,
    negative: 6.0,
    neutral: 5.0,
    positive: 10.0,
    total: 21.0
  },
  {
    date: 1520121600000,
    negative: 6.0,
    neutral: 3.0,
    positive: 12.0,
    total: 21.0
  },
  {
    date: 1523836800000,
    negative: 5.0,
    neutral: 6.0,
    positive: 9.0,
    total: 20.0
  },
  {
    date: 1524441600000,
    negative: 4.0,
    neutral: 10.0,
    positive: 5.0,
    total: 19.0
  },
  {
    date: 1524009600000,
    negative: 8.0,
    neutral: 5.0,
    positive: 4.0,
    total: 17.0
  },
  {
    date: 1525132800000,
    negative: 5.0,
    neutral: 2.0,
    positive: 8.0,
    total: 15.0
  },
  {
    date: 1523750400000,
    negative: 3.0,
    neutral: 2.0,
    positive: 9.0,
    total: 14.0
  },
  {
    date: 1524096000000,
    negative: 3.0,
    neutral: 6.0,
    positive: 5.0,
    total: 14.0
  },
  {
    date: 1515974400000,
    negative: 4.0,
    neutral: 8.0,
    positive: 2.0,
    total: 14.0
  },
  {
    date: 1524355200000,
    negative: 5.0,
    neutral: 2.0,
    positive: 5.0,
    total: 12.0
  },
  {
    date: 1524873600000,
    negative: 2.0,
    neutral: 6.0,
    positive: 0.0,
    total: 8.0
  },
  {
    date: 1525478400000,
    negative: 0.0,
    neutral: 7.0,
    positive: 1.0,
    total: 8.0
  },
  {
    date: 1524960000000,
    negative: 1.0,
    neutral: 0.0,
    positive: 1.0,
    total: 2.0
  }
];

function render_line_chart(config) {
  var margin = config.margin;
  var data = config.data;
  data = _.sortBy(data, function (d) {
    return d[config.sortby_column];
  });

  var width = $(config.selector).width() - margin.left - margin.right,
    height = 500 - margin.top - margin.bottom;

  var svg = d3
    .select(config.selector)
    .append("svg")
    .attr("width", width + margin.left + margin.right)
    .attr("height", height + margin.top + margin.bottom);

  var tooltip = d3.select("body").append("div").attr("class", "toolTip");

  var g = svg
    .append("g")
    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  if (config.sortby_column.toLowerCase().startsWith("date") ||
    config.sortby_column.toLowerCase() == "time") {
    var x = d3.scaleTime().rangeRound([0, width]);
  } else {
    var x = d3.scaleLinear().rangeRound([0, width]);
  }

  var y = d3.scaleLinear().rangeRound([height, 0]);

  var area = d3
    .area()
    .x(function (d) {
      return x(d.data[config.sortby_column]);
    })
    .y0(function (d) {
      return y(d[0]);
    })
    .y1(function (d) {
      return y(d[1]);
    });

  var stack = d3.stack();
  var keys = _.reject(Object.keys(data[0]), function (x) {
    return x == "total" || x == "Dated" || x == "date" || x == "time";
  });

  x.domain(
    d3.extent(data, function (d) {
      return d[config.sortby_column];
    })
  );

  // Find the value of the day with highest total value
  var maxDateVal = d3.max(data, function (d) {
    var vals = d3.keys(d).map(function (key) {
      if (key !== "date" && key !== "total")
        return d[key]
      else
        return 0
    });
    return d3.sum(vals);
  });

  y.domain([0, maxDateVal]);
  stack.keys(keys);
  var layer = g
    .selectAll(".layer")
    .data(stack(data))
    .enter()
    .append("g")
    .attr("class", "layer");
  
  var color = d3.scaleOrdinal(d3.schemeCategory20);

  layer
    .append("path")
    .attr("class", "area")
    .style("fill", function(d) { return color(d.key); })
    .attr("d", area)
    .on("mousemove", function (data) {
      var x0 = x.invert(d3.mouse(this)[0])

      filtered_data = data.filter(function (d) {
        var x0_date = new Date(Number(x0))
        var search_date = new Date(Number(d.data[config.sortby_column]))
        if (x0_date.getMonth() == search_date.getMonth() &&
          x0_date.getDay() == search_date.getDay() &&
          x0_date.getFullYear() == search_date.getFullYear())
          return true
        else
          return false
      })[0]

      var toolTip_html = ''
      $.each(filtered_data.data, function (index, value) {
        if (!index.toLowerCase().startsWith("date") &&
          index.toLowerCase() !== "time") {
          toolTip_html += index + ":" + value + "<br>";
        }
      })

      tooltip
        .style("left", d3.event.pageX - config.tooltip.left + "px")
        .style("top", d3.event.pageY - config.tooltip.left + "px")
        .style("display", "inline-block")
        .html(toolTip_html);
    })
    .on("mouseout", function (d) {
      tooltip.style("display", "none");
    });

  g
    .append("g")
    .attr("class", "axis axis--x")
    .attr("transform", "translate(0," + height + ")")
    .call(d3.axisBottom(x).tickFormat(d3.timeFormat(config.formatstring)))
    .selectAll("text")
    .attr("dx", config.xaxis.label_dx)
    .attr("dy", config.xaxis.label_dy)
    .attr("transform", config.xaxis.label_transform);

  g
    .append("g")
    .attr("class", "axis axis-y")
    .call(d3.axisLeft(y));
}
//date is incorrect here
var data2 = [{
    "date": 143432640000,
    "Google Chrome": 48.09,
    "Internet Explorer": 24.14,
    "Firefox": 18.82,
    "Safari": 7.46,
    "Microsoft Edge": 0.03,
    "Opera": 1.32,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143441280000,
    "Google Chrome": 48,
    "Internet Explorer": 24.19,
    "Firefox": 18.96,
    "Safari": 7.36,
    "Microsoft Edge": 0.03,
    "Opera": 1.32,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143449920000,
    "Google Chrome": 47.87,
    "Internet Explorer": 24.44,
    "Firefox": 18.91,
    "Safari": 7.27,
    "Microsoft Edge": 0.03,
    "Opera": 1.36,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143458560000,
    "Google Chrome": 48.22,
    "Internet Explorer": 23.83,
    "Firefox": 19.16,
    "Safari": 7.24,
    "Microsoft Edge": 0.04,
    "Opera": 1.39,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143467200000,
    "Google Chrome": 47.91,
    "Internet Explorer": 23.86,
    "Firefox": 19.35,
    "Safari": 7.31,
    "Microsoft Edge": 0.04,
    "Opera": 1.41,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143475840000,
    "Google Chrome": 48.78,
    "Internet Explorer": 21.14,
    "Firefox": 19.66,
    "Safari": 8.42,
    "Microsoft Edge": 0.05,
    "Opera": 1.83,
    "Mozilla": 0.1,
    "Other/Unknown": 0.01,
    "total": 100.00
  },
  {
    "date": 143484480000,
    "Google Chrome": 49.43,
    "Internet Explorer": 20.55,
    "Firefox": 19.42,
    "Safari": 8.66,
    "Microsoft Edge": 0.05,
    "Opera": 1.75,
    "Mozilla": 0.12,
    "Other/Unknown": 0.01,
    "total": 100.00
  }
]
var data3 = [{
    "date": 1383157800000,
    "Kermit": 113.44,
    "piggy": 435.15,
    "Gonzo": 19.57,
    "fuzzy": 60.55,
    "hogthrob": 53.02,
    "animal": 268.28,
    "floyd": 87.34,
    "Gabriel": 1.98,
    "Beaker": 0,
    "scooter": 31.68,
    "statler": 0,
    "waldorf": 21.59,
    "slim": 4.82,
    "sam": 11.31
  },
  {
    "date": 1385749800000,
    "Kermit": 109.86,
    "piggy": 506.85,
    "Gonzo": 18.97,
    "fuzzy": 58.77,
    "hogthrob": 388.37,
    "animal": 131.48,
    "floyd": 85.34,
    "Gabriel": 1.98,
    "Beaker": 0,
    "scooter": 48.5,
    "statler": 0,
    "waldorf": 21.59,
    "slim": 4.7,
    "sam": 11.31
  },
  {
    "date": 1388428200000,
    "Kermit": 113.46,
    "piggy": 644.88,
    "Gonzo": 19.57,
    "fuzzy": 60.57,
    "hogthrob": 669.47,
    "animal": 0,
    "floyd": 87.46,
    "Gabriel": 1.98,
    "Beaker": 0,
    "scooter": 80.97,
    "statler": 0,
    "waldorf": 21.59,
    "slim": 4.82,
    "sam": 11.43
  }
]
var config1 = {
  selector: "#issue_trend_cont",
  data: data1,
  sortby_column: "date",
  margin: {
    top: 0,
    right: 50,
    bottom: 30,
    left: 50
  },
  formatstring: "%d-%b-%y",
  xaxis: {
    label_dx: "2.5em",
    label_dy: "1.25em",
    label_transform: "rotate(60)"
  },
  tooltip: {
    left: 100,
    top: 150
  }
};

var config2 = {
  selector: "#browser_cont",
  data: data2,
  sortby_column: "date",
  margin: {
    top: 30,
    right: 50,
    bottom: 30,
    left: 50
  },
  formatstring: "%d-%b-%y",
  xaxis: {
    label_dx: "2.5em",
    label_dy: "1.25em",
    label_transform: "rotate(30)"
  },
  tooltip: {
    left: 200,
    top: 300
  }
};

var config3 = {
  selector: "#misc_cont",
  data: data3,
  sortby_column: "date",
  margin: {
    top: 50,
    right: 50,
    bottom: 30,
    left: 50
  },
  formatstring: "%d-%b-%y",
  xaxis: {
    label_dx: "2.5em",
    label_dy: "1.25em",
    label_transform: "rotate(-30)"
  },
  tooltip: {
    left: 200,
    top: 300
  }
};

$("#issue_trend_cont").empty()
$("#browser_cont").empty()
$("#misc_cont").empty()
render_line_chart(config1)
render_line_chart(config2)
render_line_chart(config3)